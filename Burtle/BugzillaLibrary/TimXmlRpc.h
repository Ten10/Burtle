...
public:
	...
	XmlRpcException(const string message) :
...
public:
	...
	public:
		...
		const XmlRpcValue& operator[](int i) const { return A[i]; }
	...
	XmlRpcValue(string const& value) : _type(TypeString) 
	...
	XmlRpcValue& operator=(const tm& rhs) { return operator=(XmlRpcValue(&rhs)); }
	...
	XmlRpcValue& operator=(XmlRpcValue && rhs);

	bool operator==(XmlRpcValue const& other) const;
	bool operator!=(XmlRpcValue const& other) const { return !(*this == other); }

	operator bool() const	{ assertTypeOrInvalid(TypeBoolean); return u.asBool; }
	operator bool&()		{ assertTypeOrInvalid(TypeBoolean); return u.asBool; }
	operator int() const	{ assertTypeOrInvalid(TypeInt); return u.asInt; }
	operator int&()			{ assertTypeOrInvalid(TypeInt); return u.asInt; }
	operator double() const	{ assertTypeOrInvalid(TypeDouble); return u.asDouble; }
	...
	operator const struct tm&()	const { assertTypeOrInvalid(TypeDateTime); return *u.asTime; }
	operator ValueStruct&()	{ assertTypeOrInvalid(TypeStruct); return *u.asStruct; }
	operator const ValueStruct&() const	{ assertTypeOrInvalid(TypeStruct); return *u.asStruct; }
	operator ValueArray&()	{ assertTypeOrInvalid(TypeArray); return *u.asArray; }
	operator const ValueArray&() const	{ assertTypeOrInvalid(TypeArray); return *u.asArray; }

	XmlRpcValue& operator[](int i)	{ assertArray(i+1); return u.asArray->at(i); }
	const XmlRpcValue& operator[](int i) const	{ assertArray(i + 1); return u.asArray->at(i); }

	XmlRpcValue& operator[](::std::string const& k) { assertStruct(); return (*u.asStruct)[k]; }
	const XmlRpcValue& operator[](::std::string const& k) const { assertStruct(); return (*u.asStruct)[k]; }

	XmlRpcValue& operator[](const char* k) { assertStruct(); ::std::string s(k); return (*u.asStruct)[s]; }
	const XmlRpcValue& operator[](const char* k) const { assertStruct(); ::std::string s(k); return (*u.asStruct)[s]; }
	...
	void toXml(ostringstream &ostr) const;

	// Formatting
	//! Return the format used to write double values.
	static string const& getDoubleFormat() { return _doubleFormat; }

	//! Specify the format used to write double values.
	static void setDoubleFormat(const char* f) { _doubleFormat = f; }

	bool parseMethodResponse(const char* s);
	void buildCall(const char* method, ostringstream &ostr) const;
...
protected:
	...
	// XML encoding
	void boolToXml(ostringstream &ostr) const;
	void intToXml(ostringstream &ostr) const;
	void doubleToXml(ostringstream &ostr) const;
	void stringToXml(ostringstream &ostr) const;
	void timeToXml(ostringstream &ostr) const;
	void binaryToXml(ostringstream &ostr) const;
	void arrayToXml(ostringstream &ostr) const;
	void structToXml(ostringstream &ostr) const;
	void nilToXml(ostringstream &ostr) const;

	// Format strings
	static string _doubleFormat;
...
class XmlRpcClient {
	class XmlRpcImplementation* secret = nullptr;
