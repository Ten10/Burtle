 ...
 *(char*)((unsigned long)MyFuncAddress + EHookA->bytes_size) = (char)0xE9;
...
void * UninstallEHook(PEHOOKSTRUCT EHookA){
  ...
  VirtualFreeEx(GetCurrentProcess, (void*)EHookA->adr_new_api, 0, MEM_RELEASE);
...
unsigned int Reckon_bytes_jmp(void * adr){
 ...
 while (5 > counter){
	 ...
	 if (c == char(0xb8)) counter += 5; //mov eax,value
 }
 return counter;
}
